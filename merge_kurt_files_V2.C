...
void merge_kurt_files_V2(const int startfile=0, const int endfile=1){
  ...
  */
  ...
  //output file:
  TFile f(Form("trig_merge_crosscheck_purdueforests_akpu3pf_v2_%d.root",endfile),"RECREATE");
...
*/
  ...
  */
  ...
  TH1F* hpPb_Jet20_v2 = new TH1F("hpPb_Jet20_v2","",1000,0,1000);
  TH1F* hpPb_Jet60_v2 = new TH1F("hpPb_Jet60_v2","",1000,0,1000);
  ...
  TH1F* hpPb_Comb_20_60_100 = new TH1F("hpPb_Comb_20_60_100","",1000,0,1000);
  ...
  TH1F* hpPb_leadjet = new TH1F("hpPb_leadjet","",1000,0,1000);
  TH1F* hpPb_leadjet_MB = new TH1F("hpPb_leadjet_MB","",1000,0,1000);
  TH1F* hpPb_leadjet_20 = new TH1F("hpPb_leadjet_20","",1000,0,1000);
  TH1F* hpPb_leadjet_40 = new TH1F("hpPb_leadjet_40","",1000,0,1000);
  TH1F* hpPb_leadjet_60 = new TH1F("hpPb_leadjet_60","",1000,0,1000);
  TH1F* hpPb_leadjet_80 = new TH1F("hpPb_leadjet_80","",1000,0,1000);
  TH1F* hpPb_leadjet_100 = new TH1F("hpPb_leadjet_100","",1000,0,1000);
  ...
  */
  ...
  TH1F* hETA = new TH1F("hETA","",1000,-5,5);

  TH1F* hJetMass = new TH1F("hJetMass","",1000,0,1000);
  TH1F* hJetPt = new TH1F("hJetPt","",1000,0,1000);
  TH1F* hTrigMass = new TH1F("hTrigMass","",1000,0,1000);
  TH1F* hTrigPt = new TH1F("hTrigPt","",1000,0,1000);

  TH1F* hJetRapidity = new TH1F("hJetRapidity","",1000,-5,5);

  TH2F* heta_vs_rap = new TH2F("heta_vs_rap","",1000,-5,5,1000,-5,5);
  ...
  TTree* jet_val = new TTree("t1","jet tree containing jet pt, mass, eta and rapidity");
  Float_t jetpt,jetmass,jeteta,jetrap;
  jet_val->Branch("jetpt",&jetpt,"jetpt/F");
  jet_val->Branch("jeteta",&jeteta,"jeteta/F");
  jet_val->Branch("jetrap",&jetrap,"jetrap/F");
  jet_val->Branch("jetmass",&jetmass,"jetmass/F");
  ...
  for(int ifile=startfile;ifile<endfile;ifile++){
    ...
    jetTree->SetBranchAddress("jtm",&jtmass);
    /*
...
*/
      ...
      hpPb_leadjet->Fill(pt3[leadJet],1);
      if(jetMB) hpPb_leadjet_MB->Fill(pt3[leadJet],L1_MB_p*jetMB_p);
      if(jet20) hpPb_leadjet_20->Fill(pt3[leadJet],jet20_p);
      if(jet40) hpPb_leadjet_40->Fill(pt3[leadJet],jet40_p);
      if(jet60) hpPb_leadjet_60->Fill(pt3[leadJet],jet60_p);
      if(jet80) hpPb_leadjet_80->Fill(pt3[leadJet],jet80_p);
      if(jet100) hpPb_leadjet_100->Fill(pt3[leadJet],jet100_p);
  ...
  //now do it for kurt's method
      ...
      for(int j = 0;j<nrefe3;j++){//start of jet loop
       ...
       hETA->Fill(eta3[j]+etashift);
       hPHI->Fill(phi3[j]);
       hJetMass->Fill(jtmass[j]);
       hJetPt->Fill(pt3[j]);

       // now that we have the pseudorapidity we need to calculate the rapidity for the jets 
       float mass = jtmass[j];
       float pt = pt3[j];
       float eta = eta3[j]+etashift;

       float rapidity = 0;

       float temp1 = (float)TMath::Sqrt(mass*mass + pt*pt*TMath::CosH(eta)*TMath::CosH(eta));
       float temp2 = (float)pt*TMath::SinH(eta);
       float temp3 = (float)TMath::Sqrt(mass*mass + pt*pt);

       rapidity = (float)log((temp1+temp2)/(temp3));

       hJetRapidity->Fill(rapidity);

       heta_vs_rap->Fill(eta,rapidity);
	...
	//first do the 12-003 merging 
	  ...
	  //cout<<"inside 12-003 method"<<endl;
         ...
         if(jet20 && !jet60 && !jet100) hpPb_Jet20_v2->Fill(pt3[j],jet20_p);
         if(jet60 && !jet100) hpPb_Jet60_v2->Fill(pt3[j],jet60_p);
       }

	//now do 12-017 
       if(fabs(eta3[j]+etashift)<1){
	  //cout<<"inside 12-017 method"<<endl;
         if(jet20 && pt3[leadJet]>=40 && pt3[leadJet]<60) hpPb_Trk40_60->Fill(pt3[j],jet20_p);
         if(jet40 && pt3[leadJet]>=60 && pt3[leadJet]<75) hpPb_Trk60_75->Fill(pt3[j],jet40_p);
         if(jet60 && pt3[leadJet]>=75 && pt3[leadJet]<95) hpPb_Trk75_95->Fill(pt3[j],jet60_p);
         if(jet80 && pt3[leadJet]>=95 && pt3[leadJet]<120) hpPb_Trk95_120->Fill(pt3[j],jet80_p);
         if(jet100 && pt3[leadJet]>=120) hpPb_Trk120->Fill(pt3[j],jet100_p);
       }
  ...
  */
  ...
  hpPb_Comb_20_60_100->Add(hpPb_Jet20_v2);
  hpPb_Comb_20_60_100->Add(hpPb_Jet60_v2);
  hpPb_Comb_20_60_100->Add(hpPb_Jet100);
  hpPb_Comb_20_60_100->Print();
